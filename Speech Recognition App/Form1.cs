...
namespace Speech_Recognition_App
{
    public partial class Form1 : Form
    {
        ...
        SpeechSynthesizer Kate = new SpeechSynthesizer();
        ...
        private void Default_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            if (speech == "Hello")
            {
                Kate.SpeakAsync("Hello, I am here");
            }

            if (speech == "How are you")
            {
                Kate.SpeakAsync("I am working normally");
            }

            if (speech == "What time is")
            {
                Kate.SpeakAsync(DateTime.Now.ToString("h mm tt"));
            }
            if (speech == "Stop talking")
            {
                Kate.SpeakAsyncCancelAll();
                ranNum = rnd.Next(1);
                if (ranNum == 1)
                {
                    Kate.SpeakAsync("Yes sir");
                }
                if (ranNum == 2)
                {
                    Kate.SpeakAsync("I am sorry i will be quiet");
                }

            }
            if (speech == "Stop Listening")
            {
                Kate.SpeakAsync("If you need me just ask");
